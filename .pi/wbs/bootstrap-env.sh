...
[ -z "$FA" ] && FA="absent"
UP=0
CODE=$(curl -s -o /dev/null -w '%{http_code}' --max-time 2 "http://127.0.0.1:8000/api/v1/config/status" 2>/dev/null)
[ "$CODE" = "200" ] && UP=1
AG=""
if [ "$UP" = "1" ]; then
  AG=$(curl -s --max-time 2 "http://127.0.0.1:8000/api/v1/agents" 2>/dev/null | "$PY" -c 'import sys,json;print(len(json.load(sys.stdin)))' 2>/dev/null)
fi
[ -z "$AG" ] && AG=0
echo "uv: ${UV}"
echo "fastapi: ${FA}"
echo "server_8000: up=${UP}"
